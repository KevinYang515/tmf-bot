...
**核心判斷**：執行漏單問題若不修，升 MXF 等於把錯誤放大 5 倍。

### 帳戶 size 需求

| 帳戶承受 DD | 帳戶最少要 | + 4 口 MXF margin | 推薦帳戶 size |
|-------------|------------|-------------------|---------------|
| 5%（極保守）| NT$ 3.76M | +180K | 4M |
| **10%（推薦）** | NT$ 1.88M | +180K | **2.06M** |
| 20%（積極）| NT$ 940K | +180K | 1.12M |

目前帳戶 NT$ 186K → **離可升 MXF 的最低門檻（1.12M）還差約 1M**。

### 預期年化 ROI

| 實現率假設 | TMF (現況) | MXF (升級後) |
|------------|------------|--------------|
| 25%（目前，沒修漏單）| +83K / 年 | +400K / 年 |
| 50%（修一半）| +166K | +800K |
| **70%（修完）** | **+232K** | **+1.12M** |

### 升 MXF 的必要條件（缺一不可）

1. ✅ TV alert 加 placeholder（已做 2026-06-22）
2. ⏳ webhook_raw.csv 1 個月資料 review
3. ⏳ 修漏單成因（改 pine 或 app.py）
4. ⏳ 修完 TMF 跑 1 個月，實現率 ≥ 50%
5. ⏳ 連續 2 個月正 PnL
6. ⏳ 帳戶加碼到 NT$ 1.5M+
7. ⏳ 先試 1 口 MXF + 對應減 TMF 跑 2 週
