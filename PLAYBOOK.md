...
⚠️ 注意：實盤另有 bot 之外的手動 hedge 空單（浮虧約 -232K @ 2026-07-07），不計入程式單績效；trade_records 有 7 處部位跳號 + 17 筆 Failed，對帳機制仍待修。
...
## 7B. MXF 升級評估（更新 2026-07-07：數據面已達標，剩營運前置）

**2026-07-07 重評**：正確顆粒度驗證後，執行捕捉率正常日接近 100%（見第 7 節更新），「實現率 25%」為舊誤判。帳戶已補資至權益 ~1,146K。

### 帳戶 size 需求（DD 預算改用實測 -400K，MXF 原始保證金 159K/口 @ 2026-07）

| 帳戶承受 DD | 帳戶最少要 | + 4 口 MXF margin (636K) | 推薦帳戶 size |
|-------------|------------|--------------------------|---------------|
| 20% | NT$ 2.0M | ✓ 含 | 2.0M |
| **27%（現實）** | **NT$ 1.5M** | ✓ 含 | **1.5M** |
| 35%（積極，現況）| NT$ 1.15M | ✓ 含 | 1.15M — 可做但 DD 來時緊 |

### 預期獲利（依 7/7 驗證的捕捉率 85-90%）

- TV 理論近 16 週：+1.69M MXF ≈ 10.5萬/週 → 打 85 折 ≈ **9萬/週（MXF 4 口滿倉配置）**
...
4. ✅ 帳戶 ≥ 1.15M（積極門檻）
5. ⏳ **修對帳/告警**：Failed 單 retry+推播、成交後與 Shioaji 實際部位比對、60 秒反向訊號 debounce（7 處跳號 + 17 筆 Failed 的根治）
6. ⏳ **處理手動 hedge 空單**（浮虧 -232K，佔用保證金）
7. ⏳ 切換時 **pyramiding 先設 2** 跑 2 週對帳無誤 → 開回 4（註：加碼單是主要獲利來源，cap 2 有期望值成本，純為過渡期風控）
...
- [ ] **對帳/告警（升 MXF 前置）：** app.py 加 (1) Failed 單 retry + 推播 (2) 成交後與 Shioaji 實際部位比對告警 (3) 60 秒反向訊號 debounce
- [ ] **手動 hedge 空單處理：** 帳上 bot 之外的空單浮虧 ~-232K（2026-07-07），平掉或設停損
- [ ] **P&L 核實：** 與永豐月結單對比確認系統損益計算正確（建議每月底執行）
- [ ] **MXF 升級評估：** 2026-07-07 重評後數據面已達標（見 §7B），剩對帳/告警與 hedge 兩項前置
